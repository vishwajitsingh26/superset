...
def fallback_dataset(binding_set: dict[str, Any]) -> int | None:
    ...
    nothing -- a title, a static caption. Stage B does not bind those regions
    and should not, so the id has to come from somewhere: the dataset the rest
    of the dashboard already uses. The most-bound one, so a run spanning two
    datasets attaches the caption to the dominant one rather than to whichever
    binding happened to sort first.
    """
    counts: dict[int, int] = {}
    for binding in binding_set.get("bindings", []):
        dataset_id = binding.get("dataset_id")
        if isinstance(dataset_id, int) and not isinstance(dataset_id, bool):
            if dataset_id > 0:
                counts[dataset_id] = counts.get(dataset_id, 0) + 1
    if not counts:
        return None
    return max(sorted(counts), key=lambda dataset_id: counts[dataset_id])
...
def build_user_prompt(
    ...
    if hint := decision.get("params_hint"):
        # This viz type was generated by stage F in this same run, so its author
        # knows exactly which controls it defines.
        extra = (
            "\n\nThe plugin for this viz type was built for this design in this "
            "run. Its author supplied the params it expects; treat these as "
            "authoritative and fill in the region's real values:\n"
...
def validate(  # noqa: C901
    ...
    for derivation in binding.get("derivations") or []:
        allowed |= _names_of(derivation)
    ...
    if children := decision.get("children") or []:
        ...
        for child in children:
            if f"__REF__:{child}" not in blob:
                problems.append(f"wrap child {child!r} not referenced as __REF__")
    ...
    def binding_for(region_id: str) -> dict[str, Any]:
        ...
        for key in sorted(bindings):
            if key.split(":")[0] == parent:
                return bindings[key]
        # Nothing bound this region, and stage B was right not to: a header or
        # a text block reads no data. It still becomes a chart, and Superset
        # requires a datasource on every chart, so it is attached to the one
        # the rest of the dashboard uses. Marked so the worker can tell this
        # apart from a dataset chosen for it.
