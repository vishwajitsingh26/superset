...
def build_user_prompt(
    ...
    naming = (
        (
            "\n\n**This run's directory suffix is "
            f"`-{tag}`.** Your `directory` must end with it: "
            f"`superset-frontend/plugins/plugin-chart-custom-<name>-{tag}`, and "
            "every file path sits inside that directory. It marks the plugin as "
            "generated by this run so it can be found and cleaned up later.\n"
            "Put the suffix **nowhere else**: not in `package_name`, not in "
            "`viz_type`, not in the chart's display name. Those are what a user "
