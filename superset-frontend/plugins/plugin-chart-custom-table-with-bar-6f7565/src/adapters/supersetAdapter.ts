...
//
// Extended from the seeded version with `validateNonEmpty` (required by the
// control panel's own validators) and the `QueryFormOrderBy` type (needed to
// type buildQuery's descending sort) -- every symbol the seed already
// exported is kept unchanged below.
import {
  Behavior,
  ChartMetadata,
  ChartPlugin,
